...
export function activate(context: vscode.ExtensionContext) {
	...
	const catDecoration = vscode.window.createTextEditorDecorationType({
    gutterIconPath: vscode.Uri.file(path.join(context.extensionPath, 'media', 'angy_cat.jpeg')),
	...
	diagnostics.forEach(diagnostic => {
		if (diagnostic.severity === vscode.DiagnosticSeverity.Error) {
			errorRanges.push({
				...
				hoverMessage: '😾 Cat detected an error!'
			});
		}
	});

	activeEditor.setDecorations(catDecoration, errorRanges);
} else {
	activeEditor.setDecorations(catDecoration, []); // Clear if no errors
}
